import {CodeContainer, CodeEditor, CodeStyle, NumberListRow, TabsContainer, TabStyle} from "./code.styled.ts";
import CodeHighlighter from "./functions/CodeHighlighter.ts";
import HTMLString from "react-html-string";

export default function Code() {
  const code = `function App() {
      const name = 'André Lucas';
      const skills = ['NextJS', 'Nest.JS'];
      const age = 20;
...
`;
  ...
  return (
    <CodeContainer>
      ...
      <CodeStyle>
        <NumberListRow>
          {element.lineOfCode.map((_e,i) => {
            return (<p key={i + 1}>{i + 1}</p>)
          })}

        </NumberListRow>
        <CodeEditor>
          <HTMLString html={element.code}/>
        </CodeEditor>
      </CodeStyle>
    </CodeContainer>
  )
